<script setup lang="ts">
  ...
  const formSchema = yup.object({
    ...
    // 주소 관련 필드
    zipCode: yup.string().required('⚠️ 우편번호를 입력해주세요'),
    address: yup.string().required('⚠️ 주소를 입력해주세요'),
    addressDetail: yup.string().required('⚠️ 상세주소를 입력해주세요'),
    latitude: yup.string().required('⚠️ 위도를 입력해주세요'),
    longitude: yup.string().required('⚠️ 경도를 입력해주세요'),
  })
    ...
    <div class="mx-auto max-w-2xl">
      ...
      >
        ...
        <form class="grid max-w-[720px] gap-4" @submit.prevent="onSubmit">
          ...
          <!-- 주소 검색 -->
          <div>
            <p class="text-grey-900 mb-2 text-sm font-bold">주소</p>
            <FormAddressInputs
              zpcd-name="zipCode"
              addr-name="address"
              dtl-addr-name="addressDetail"
              lat-name="latitude"
              lng-name="longitude"
              size="max"
            />
